...
can be used. There are five additional tags available: \verb/<pkg>/, \verb/<view>/,
\verb/<bioc>/, \verb/<forge>/, \verb/<googlecode>/, and \verb/<code>/. \verb/<pkg>/ and
...
hyperlinks to the packages/views. Similarly, \verb/<bioc>/, \verb/<forge>/, and \verb/<googlecode>/
can be used to refer to projects on Bioconductor, \textsf{R}-Forge, and Google Code.
These will be replaced by the appropriate hyperlinks to the project on
\texttt{http://www.Bioconductor.org/}, \texttt{http://R-Forge.R-project.org/},
and\linebreak \texttt{http://Code.Google.com/}, respectively.
